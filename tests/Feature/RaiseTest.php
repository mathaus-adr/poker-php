...
class RaiseTest extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_if_can_raise_and_store_entities_with_correct_values(): void
    {
        ...
        $this->assertDatabaseHas(RoundAction::class, ['action' => 'raise', 'user_id' => $user->id, 'room_round_id' => $round->id, 'amount' => 20]);
        $this->assertDatabaseMissing(RoomRound::class, ['player_turn_id' => $user->id]);
        $this->assertDatabaseHas(RoomRound::class, ['total_players_in_round' => $totalPlayers, 'total_pot' => 35, 'current_bet_amount_to_join' => 25]);
        $round->refresh();
        $payAction = app(Pay::class);
//        dd($round->player_turn_id);
        $payAction->execute($round->room, User::find($round->player_turn_id));
        $this->assertDatabaseHas(RoomRound::class, ['total_players_in_round' => $totalPlayers, 'total_pot' => 60, 'current_bet_amount_to_join' => 25]);
    }
}
